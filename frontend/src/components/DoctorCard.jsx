...
const DoctorCard = ({ item }) => {
  ...
  return (
    <div
      onClick={goToBooking}
      className="group relative bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-transform transform hover:-translate-y-1 cursor-pointer"
    >
      {/* Top Section with Image and Availability */}
      <div className="relative h-48 bg-gray-50 flex items-end justify-center">
        <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1 bg-white rounded-full shadow text-sm">
          <span
      ...
      <div className="p-5 space-y-4 text-center">
        ...
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="w-full py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold flex items-center justify-center gap-3 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          onClick={(e) => {
            e.stopPropagation();
            goToBooking();
          }}
        >
          <span>Book Now</span>
          <FaRegCalendarCheck className="text-lg group-hover:translate-x-1 transition-transform duration-300" />
        </motion.button>
      </div>
    </div>
  );
